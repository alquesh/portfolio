...
app.get('/tweets', function(req,res,next) {
    res.json({
        "href": "http://localhost:3000/tweets",
...
app.get('/tweets/:id', function(req,res,next) {
    res.json({
        "href": "http://localhost:3000/tweets/"+ req.params.id,
...
app.get('/users', function(req,res,next) {
	...
	//preliminary route to "/users?expand=tweets"
    if(req.query.expand == "tweets") {
        var temp = store.select('users');
        for (var i = 0; i < temp.length; i++) {
            temp[i].tweets = {};
            temp[i].tweets.href = "http://localhost:3000/users/"+temp[i].id+"/tweets";
            var tweets = store.select('tweets');
            temp[i].tweets.items = [];
            for (var x = 0; x < tweets.length; x++) {
                if (tweets[x].creator.href == "http://localhost:3000/users/"+temp[i].id) {
                    tweets[x].href = "http://localhost:3000/tweets/" + tweets[x].id;
                    temp[i].tweets.items.push(tweets[x]);
                }
            }
            temp[i].href = "http://localhost:3000/users/" + temp[i].id;
        }
        res.json({
...
app.get('/users/:id', function(req,res,next) {
    res.json({
        ...
        "items": store.select('users', req.params.id),
        "tweets": {
            "href":"http://localhost:3000/users/"+ req.params.id+"/tweets"
        }
    });
});

//store does not implement support for this route
